%% PRELIMINARIES
% imaging system
sigma = 1;      % rayleigh limit

% parameters (as column vectors)
x0 = sigma*(0:0.02:0.2)';        % geometric midpoint
%s = sigma*linspace(1e-3,.5,100)';     % half-separation 
s = sigma*10.^linspace(-3,0,100)';
...
% calculate quantum cramer rao bound
QCRB = pageinv(Q);
...
I_DD(1:2,1:2,:,:,:) = I_DD(1:2,1:2,:,:,:)/sigma; % normalize physical dimensions

% Compute the CRB matrix for direct imaging
CRB_DD = pageinv(I_DD);

% MATHEMATICA OUTPUT
% Compute the CFIM in the sub-diffraction approximation
...
I_DD_approx(1:2,1:2,:,:,:) = I_DD_approx(1:2,1:2,:,:,:)/sigma; % normalize physical dimensions


%% HERMITE GAUSS MODE SORTING CFIM PLOTS
...
I_HG(1:2,1:2,:,:,:) = I_HG(1:2,1:2,:,:,:)/sigma; % normalize physical dimensions
...
% Plot the diagonal elements of the Fisher Information Matrix
figure
...
title(t4,{'Diagonal Fisher Information Matrix Elements','HG SPADE',sprintf('$x_0 / \\sigma=%.f$',x0(1))},'interpreter','latex')
%}
...
title(t5,{'Diagonal Fisher Information Matrix Elements','HG SPADE',sprintf('$\\kappa=%.f$',kappa(1))},'interpreter','latex')
...
figure
t9=tiledlayout(2,3,'TileSpacing','compact','Padding','compact');
n=1;
for i=1:3
    for j=i:3
        %nexttile(sub2ind([3,3],j,i))
...
title(t9,{'Analytic v. Numeric CFIM','HG SPADE',sprintf('$\\kappa=%f.$',kappa(1))},'interpreter','latex')
...
% Plot the diagonal elements of the Fisher Information Matrix
figure
t8 = tiledlayout(1,3,'TileSpacing','compact','Padding','compact');
for i = 1:3
    nexttile(i)
    hold on
    for k = 1:numel(kappa)
    ...
    hold off
    xlabel('$s/\sigma$','interpreter','latex')
    ylabel(['Fisher Information ','$Q_{',parameter_names{i},'}(s)/\sigma^2$'],'interpreter','latex')
    if i==3
        leg=legend(kappa_legend_names);
        leg.Location='NorthWest';
        title(leg,'$\kappa$','interpreter','latex')
...
title(t8,{'Diagonal Quantum Fisher Information Matrix Elements'},'interpreter','latex')
